...
void Displayer::generateBlockDisplayList (   )
{
  for (size_t i = 0; i < BF_NUMBER; ++i) {
    ...
    special_block_list[i] = glGenLists(1);
    glNewList(special_block_list[i], GL_COMPILE);
    ...
    small_block_list[i] = glGenLists(1);
    glNewList(small_block_list[i], GL_COMPILE);
      glPushMatrix();
        glRotatef(45.0f, 1.0f, 0.0f, 0.0f);
        glRotatef(45.0f, 0.0f, 1.0f, 0.0f);
        glScalef(0.5f, 0.5f, 0.5f);
        glEnable(GL_NORMALIZE);
        glCallList(block_list[i]);
